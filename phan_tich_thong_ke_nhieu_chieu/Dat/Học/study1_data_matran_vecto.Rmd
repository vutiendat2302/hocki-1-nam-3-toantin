...
- Không đặt tên biến số bằng ký hiệu - | _ Vd: my_object
-  Hỗ trợ trong R: help(), help(Phương thức, hàm cần tìm hiểu), ?phương thức, hàm cần tìm hiểu

```{r}
# Sự dụng data_frame(x, y): để ghép 2 cột y, y
# Chúng ta cũng có thể nhập data trong bảng bằng edit(data.frame())
...
# dùng edit(data.frame()) để nhập nhanh hơn
...
# Kiểm tra với dữ liệu có trống không
# Loại bỏ các dữ liệu trống trong dữ liệu Indometh
data1 = na.omit(Indometh)
data1

# Tách dữ liệu từ dữ liệu gốc với điều kiện
x = Indometh$time[Indometh$time > 1]
x
y = subset(Indometh$time, Indometh$time > 1)
y

# Nhập dữ liệu ngày tháng 
# mặc định của date là: yyyy-mm-dd
...
M1 %*% M2    # nhân 2 với nhau ma trận (AB là dùng cái này chứ ko phải cái * bình thường này đâu nhé)
t(M1)        # chuyển vị
...
```{r}
#1. prod: Hàm dùng để tính tích các phần tử trong một vector hoặc chúng ta cũng có thể dùng nó để tính giai thừa ví dụ 5!
prod(5:1)
